 ...
 const handleUpload = async (e: any) => {
  ...
  for (const file of files) {
    ...
    const res = await fetch(
      `http://localhost/managerbp/public/seller/departments/upload.php?user_id=${userId}`,
      {
        method: "POST",
        body: formData,
      }
  ...
  // Push all uploaded images at once
        ...
        <div className="grid grid-cols-3 gap-4 mt-4">
          {images.map((img, index) => {
            const imgSrc = img.startsWith("http")
              ? img
              : `http://localhost/managerbp/public/uploads/${img}`;
