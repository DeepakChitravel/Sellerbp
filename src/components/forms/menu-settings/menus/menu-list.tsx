...
export default function MenuList() {
  ...
  /* ================= GET STATUS COLOR ================= */
  const getMenuStatus = (orders: number) => {
    if (orders === 0) return { text: "New", color: "bg-gray-100 text-gray-800" };
    if (orders < 10) return { text: "Popular", color: "bg-blue-100 text-blue-800" };
    if (orders < 50) return { text: "Hot", color: "bg-orange-100 text-orange-800" };
    return { text: "Best Seller", color: "bg-red-100 text-red-800" };
  };
      ...
      <div className="mb-8">
        <div className="flex justify-between items-center">
          ...
          <button
            ...
            className="bg-gradient-to-r from-purple-600 to-purple-700 hover:from-purple-700 hover:to-purple-800 text-white font-medium py-3 px-6 rounded-xl flex items-center gap-2 transition-all duration-200 shadow-lg hover:shadow-xl"
          >
        ...
        <div className="flex items-center gap-6 mt-8 text-sm text-gray-600">
          ...
          <div className="flex items-center gap-2">
            ...
            <span>{menus.filter(m => m.orders > 0).length} Active</span>
          </div>
          <div className="flex items-center gap-2">
            <div className="w-3 h-3 rounded-full bg-green-500"></div>
            <span>{menus.filter(m => m.totalAmount > 0).length} Revenue Generating</span>
          </div>
        </div>
      </div>
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">
            Scroll to view all menus. Click on any menu to manage items.
          </p>
        </div>
          ...
          {loading ? (
            ...
            Array.from({ length: 4 }).map((_, index) => (
              <div key={index} className="flex-shrink-0 w-[340px]">
                <div className="bg-white rounded-2xl border p-6 h-[280px] animate-pulse">
            ...
            <div className="w-full max-w-2xl mx-auto text-center py-16">
              ...
              <h3 className="text-2xl font-bold text-gray-900 mb-3">No menus created yet</h3>
              <p className="text-gray-600 mb-8 max-w-md mx-auto">
                Start by creating your first menu. Add items, set prices, and track orders all in one place.
              </p>
              <button
                onClick={() => setOpen(true)}
                className="bg-gradient-to-r from-purple-600 to-purple-700 hover:from-purple-700 hover:to-purple-800 text-white font-medium py-3 px-8 rounded-xl transition-all duration-200 shadow-lg hover:shadow-xl"
              >
            ...
            // Menu Cards
            menus.map(menu => {
              const status = getMenuStatus(menu.orders);
              return (
                <div key={menu.id} className="flex-shrink-0 w-[340px]">
                  <div className="bg-white rounded-2xl border border-gray-200 p-6 hover:shadow-xl transition-all duration-300 hover:border-purple-200 group">
                    {/* Header with Status */}
                      ...
                      <div className="bg-orange-50 rounded-xl p-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-xl font-bold text-gray-900">
                              ${menu.orders > 0 
                                ? (menu.totalAmount / menu.orders).toFixed(0)
                                : '0'}
                            </p>
                          </div>
                        </div>
                    ...
                    <div className="flex gap-3 pt-4 border-t border-gray-100">
                      <button
                        ...
                        className="flex-1 py-2.5 text-sm font-medium text-gray-700 hover:text-purple-700 hover:bg-purple-50 rounded-lg transition-colors"
                      >
                        Edit
                      </button>
                      <button
                        onClick={() => refreshMenuStats(menu.id)}
                        className="flex-1 py-2.5 text-sm font-medium text-gray-700 hover:text-blue-700 hover:bg-blue-50 rounded-lg transition-colors"
                      >
                        Refresh
                      </button>
                      <button
                        onClick={() => handleDeleteClick(menu.id)}
                        className="flex-1 py-2.5 text-sm font-medium text-gray-700 hover:text-red-700 hover:bg-red-50 rounded-lg transition-colors"
                      >
                        Delete
                      </button>
                    </div>
